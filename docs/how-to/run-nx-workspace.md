...
rtk npm run check:harness-parity
...
`rtk ./hippo run --class ephemeral --resource-tier standard --disk-path . -- npm exec -- nx show projects`. This
repository uses only raw Nx command targets; see the
