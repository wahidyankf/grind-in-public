...
Rule-path automation triggers this workflow before a supported harness edits a rule and again at pre-commit. Do not wait for an owner to name the workflow: its trigger starts this procedure. The [rule change trigger policy](../../development/rule-change-trigger-policy.md) owns the rule paths, the hooks that watch them, and when [Harness Alignment](../harness-alignment.md) is triggered alongside this workflow.
