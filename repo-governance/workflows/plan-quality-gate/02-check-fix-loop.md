...
The checker verifies the plan against itself and the current repository: a command named in `delivery.md` must exist; every checklist path appears in `tech-docs/file-impact.md`; durable scenarios bind both ways between `prd.md`, `specs/`, and a matching RED step; and a plan-only outcome has a reason and delivery proof. It compares planned C4 and Gherkin claims with current specifications, implementation, governance, and active plans. It also applies [Plan Minimum Sufficiency](05-minimum-sufficiency.md).
