...
Write the five core documents in dependency order, because each one constrains the next. The [five-document structure](../../conventions/plans-organization-policy/five-document-structure.md) defines what each file owns.
...
2. **`prd.md`** — user stories and Gherkin acceptance criteria, each with a stable `[AC-…]` label, per the [specs policy](../../development/specs-policy.md). Mark any rollout or operational criterion that remains plan-only and give its reason and delivery proof; durable behavior scenarios land in `specs/`.
3. **`tech-docs/README.md`** — architecture, design decisions with rationale, dependencies, risks, rollback path, and technical reading order. Write `file-impact.md` with every exact path and `[E]`, `[N]`, `[M]`, or `[D]` label; add the applicable specification-change, migration, and UI-design companions before authoring delivery.
...
5. **`README.md`** — written last despite being read first: context, scope with projects named, the approach in a paragraph, and a map linking every plan artifact.
