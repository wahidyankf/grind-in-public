...
5. **Proposed direction** — at most three elements, sketched at a level a reader grasps immediately. Not file paths, not signatures, not Gherkin: that detail is the plan's job.
6. **Scope and non-goals** — in-scope bullets plus an explicit out-of-scope list. A non-goal names something a reader would reasonably expect and excludes it deliberately.
7. **Risks and open questions** — the rabbit holes worth flagging now and the unknowns that block promotion. Zero open questions means the idea is either over-specified or under-thought.
8. **Success and promotion signal** — what would make pursuing this worthwhile, and what "ready for `backlog/`" means for this specific idea. State an observable fact or a labeled judgment call; never invent a metric.
