...
Each harness asks `scripts/check-rule-change.mjs` for the notice rather than keeping its own copy of the rule paths, so
the three cannot drift apart: all of them call `harness rule-change hook`. The payloads differ and the command reads
both, since Claude Code names the file while Codex sends a patch whose file headers it parses.
