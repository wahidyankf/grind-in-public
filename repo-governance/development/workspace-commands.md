...
`wahidyankf-www-e2e` owns no `test:quick`, so `npm run test:quick` and the pre-push affected run reach the other two
projects only. That is the shape the [testing policy](testing-policy.md) gives a permitted dedicated project and the
[BDD policy](behavior-driven-development-policy.md) role matrix requires of one, not a gap. Run
`wahidyankf-www-e2e:install` once per machine before `test:e2e`, which builds and starts `wahidyankf-www` itself.
