...
directive and `go.sum` follow [Native-First Toolchain](../../testing-policy/tooling.md) and Reproducibility.
...
Findings fail at the threshold [Lint Strictness](../../testing-policy/tooling.md) sets, and each `//nolint` names its
...
[Test Boundaries and Gates](../../quality-gates.md) classifies it.
...
Every exported name has a doc comment beginning with that name. An exported sentinel or error type joins the Public
Contract, so one is exported only when a caller needs it, and a behaviour change updates its documentation per
[Specification Maintenance](../../specs-policy.md).
...
|                       | a library; example: testify | shorter assertions, clear diffs | a dependency, per [Dependency Selection](../../dependency-selection-policy.md) |
