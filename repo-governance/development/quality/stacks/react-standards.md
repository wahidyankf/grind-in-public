...
It implements Pure Functions, Explicit Over Implicit, Accessibility First, and Simplicity Over Complexity. Typing
follows [TypeScript Standards](typescript-standards.md), interfaces follow Accessibility and Design Tokens, and a
Next.js application adds [Next.js Standards](nextjs-standards.md).

## Version

New work targets the current stable major line, pinned as [Native-First Toolchain](../../testing-policy/tooling.md)
requires, unless the framework it runs on records the major, as Next.js Standards does. Crossing a major is a planned
change.
...
- **Logic stays outside components.** Domain rules live in plain functions, per Functional Core, Imperative Shell. A
  custom hook adapts them to components and maps domain errors to messages a user can act on. Components are the
  presentation adapter Application Shapes describes.
...
manual Release Check. Layers and test-first work follow [Test Boundaries and Gates](../../quality-gates.md) and
[Test-Driven Development](../../tdd-policy.md).

## Gates

Beyond the TypeScript gates, lint enables the rules of hooks, exhaustive effect dependencies, and JSX accessibility
rules, failing at the threshold [Lint Strictness](../../testing-policy/tooling.md) sets.
