...
The canonical as-built boundaries for `wahidyankf-www` live in [architecture.md](architecture.md), and its executable behavior lives in [behavior/](behavior/README.md). Eleven feature files carry 53 scenarios.

Two projects share this corpus, as the [architecture specification policy](../../../repo-governance/development/architecture-specifications.md) provides for: the application at `apps/wahidyankf-www` and the dedicated E2E project at `apps/wahidyankf-www-e2e`.
...
The E2E adapter deliberately leaves four features unbound — `env-loader.feature`, `tier-env-loading.feature`, `port-resolver.feature`, and `cv-export.feature` — because they specify Node-process environment concerns and a build-time export that no browser reaches. That gap is not left to be noticed: `apps/wahidyankf-www-e2e` records a generated skip baseline and fails when the number moves, so a newly broken binding cannot hide among the intended skips.
