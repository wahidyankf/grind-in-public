...
Badak Mini's canonical as-built boundaries live in [architecture.md](architecture.md), and its executable behavior lives
in [behavior/](behavior/). Every feature is run unchanged by its unit, local-integration, and public-process E2E
adapters.
...
The owner's `test:coverage:behavior` target verifies recursive discovery, expression binding, and adapter parity. Its
co-located E2E package uses the same corpus and owns no features of its own.
