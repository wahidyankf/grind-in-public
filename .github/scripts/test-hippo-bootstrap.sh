...
jq -e '.schemaVersion == 1 and .source == "grind-in-public"' "$repository_root/hippo.identity.json" >/dev/null
...
if grep -q '^DOTNET_PROCESSOR_COUNT$' "$arguments_file"; then
	exit 1
fi
...
# title validation; this repository keeps no copied feature corpus. The list is
# exact, so
