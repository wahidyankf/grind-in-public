...
Then('the H1 shows "Welcome to My Portfolio"', async ({ page }) => {
  await expect(
    page.getByRole("heading", { level: 1, name: /Welcome to My Portfolio/ }),
  ).toBeVisible();
});
...
Then(
  ...
  async ({ page }) => {
    await expect(
      page.getByText(/Top Skills Used in The Last 5 Years/i),
    ).toBeVisible();
  },
);
...
Then(
  ...
  async ({ page }) => {
    await expect(
      page.getByText(/Top Programming Languages Used in The Last 5 Years/i),
    ).toBeVisible();
  },
);
...
Then(
  ...
  async ({ page }) => {
    await expect(
      page.getByText(/Top Frameworks & Libraries Used in The Last 5 Years/i),
    ).toBeVisible();
  },
);
...
Then(
  ...
  async ({ page }) => {
    const link = page.getByRole("link", {
      name: /Browse My Independent Projects/i,
    });
    await expect(link).toBeVisible();
...
Then("a Connect With Me card is visible", async ({ page }) => {
  await expect(
    page.getByRole("heading", { name: /Connect With Me/i }),
  ).toBeVisible();
});
