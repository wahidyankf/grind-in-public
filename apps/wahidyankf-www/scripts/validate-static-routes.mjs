...
const routesManifest = JSON.parse(
  readFileSync(resolve(appRoot, ".next/routes-manifest.json"), "utf8"),
);
const publicRoutes = [
  "/",
  "/cv",
...
function requireRoutes(routes, manifestName) {
  ...
  if (missingRoutes.length > 0) {
    throw new Error(
      `${manifestName} is missing static route(s): ${missingRoutes.join(", ")}`,
    );
  }
}
...
if (dynamicRoutes.length > 0) {
  throw new Error(
    `routes-manifest.json contains dynamic route(s): ${dynamicRoutes.join(", ")}`,
  );
}

// This validator runs under `static-routes:validation`, and this line is its
// only report of what it checked. A validator that passes silently cannot be
// told from one that examined nothing.
// biome-ignore lint/suspicious/noConsole: a validator reports what it verified on stdout
