...
export const cvData: CVEntry[] = [
  {
    ...
    details: [
      ...
      "Work bridges technical architecture and business strategy: from cyber-security hardening and automated threat intelligence to payment integrations with Indomaret, Indonesia's nationwide convenience-store network. Builds AI-augmented delivery systems and agent-orchestrated engineering workflows with quality guardrails, in production and in the open through Open Sharia Enterprise (OSE), an open-source initiative for ethical, Sharia-compliant enterprise software. Builds high-trust, high-performance teams for reliable financial technology, and shares software-engineering and Islamic-fintech insights through a blog and YouTube channel, AyoKoding (ayokoding.com).",
      "Based in Jakarta, works across Indonesia and internationally, and is open to relocation for the right software-engineering opportunity.",
    ],
    links: {
  ...
  },
  {
    ...
    details: [
      "One year of German-language study ahead of postgraduate studies in Germany",
    ],
    type: "education",
  },
  {
  ...
  data
    ...
    .forEach((entry) => {
      ...
      selectItems(entry)?.forEach((name) => {
        let info = itemInfo[name];
        if (!info) {
          info = { usedInWindow: false, periods: [] };
          itemInfo[name] = info;
        }
        if (end >= fiveYearsAgo) {
...
export const getTopFrameworksLastFiveYears = (
  data: SkillSource[],
): SkillTotal[] => topItemsLastFiveYears(data, (entry) => entry.frameworks);

export const getTopAISkillsLastFiveYears = (
  data: SkillSource[],
): SkillTotal[] => topItemsLastFiveYears(data, (entry) => entry.aiSkills);
