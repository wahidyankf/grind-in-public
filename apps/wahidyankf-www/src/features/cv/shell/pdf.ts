...
const addSections = (
  ...
  entries.forEach((entry) => {
    pdf
      ...
      .text(entry.title);
    const subtitle = [entry.organization, entry.period]
      .filter(Boolean)
      .join(" · ");
    if (subtitle) {
      pdf
    ...
    entry.meta.forEach((line) => {
      pdf.fontSize(9.5).fillColor(COLORS.gray).text(line, { lineGap: 2 });
    });
    pdf.moveDown(0.6);
  });
};
...
const addFooters = (pdf: PDFKit.PDFDocument, name: string): void => {
  ...
  for (let i = range.start; i < range.start + range.count; i += 1) {
    ...
    pdf
      ...
      .text(
        `${name} | Page ${i - range.start + 1}`,
        left,
        pdf.page.height - FOOTER_MARGIN_FROM_BOTTOM,
        {
...
export const renderCvPdf = (
  document: CvPdfDocumentModel,
): PDFKit.PDFDocument => {
  ...
  pdf
    ...
    .text(document.tagline, { align: "center" });
  pdf.moveDown(0.3);
  pdf
    .font("Helvetica")
    .fontSize(10)
    .fillColor(COLORS.black)
    .text(document.contactLine, { align: "center" });
  pdf.moveDown(0.8);
