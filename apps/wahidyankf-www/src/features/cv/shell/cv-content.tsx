...
const CVEntryComponent = ({
  ...
  <div className="mb-4 border border-green-400 p-4">
    ...
    {entry.type === "work" ? (
      ...
      entry.details.map((detail) => (
        <p key={detail} className="mb-2 text-green-200">
          {parseMarkdownLinks(detail, searchTerm)}
        </p>
      ))
    )}
      ...
      <>
        ...
        {entry.programmingLanguages && (
          <div className="mt-2">
            ...
            <ul className="mb-2 grid list-none grid-cols-2 gap-2">
              {entry.programmingLanguages.map((lang) => (
                <li key={lang}>
                  <ClickableItem
                    name={lang}
                    duration={0}
...
const CVSection = ({
  ...
  return (
    <div className="mb-8">
      ...
      {entries.map((entry) => (
        <CVEntryComponent
          key={`${entry.type}-${entry.organization}-${entry.period}-${entry.title}`}
          entry={entry}
          searchTerm={searchTerm}
...
const WorkExperienceSection = ({
  ...
  const groupedEntries = entries.reduce(
    (acc, entry) => {
      let bucket = acc[entry.organization];
      if (!bucket) {
        bucket = [];
        acc[entry.organization] = bucket;
      }
      bucket.push(entry);
      return acc;
    },
    ...
    <div className="mb-8">
      <StickyHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center">
            <span className="mr-2 text-sm text-green-300">
            ...
            <button
              ...
              aria-label={
                showRecentOnly
                  ...
                  : "Show recent work experience only"
              }
            >
              {showRecentOnly ? (
...
/**
 * Renders the whole CV page: the search box, the recent-only filter, and every
 * dated section below them. The search term is mirrored into the URL query, so
 * a filtered view can be linked to and survives a reload; that mirroring is
 * why this component holds a router rather than keeping the term in local
 * state alone.
 */
export function CvContent() {
  ...
  useEffect(() => {
    if (
      new URLSearchParams(window.location.search).get("scrollTop") === "true"
    ) {
      window.scrollTo(0, 0);
  ...
  const skillSources = [
    ...cvData.filter((entry) => entry.type === "work"),
    ...projects,
  ];
