...
Badak Mini currently has strong Go tests, but `specs/` is empty and no automated check proves that declared behavior
stays executable across unit, integration, and public process boundaries. That leaves behavior discussions detached from
the test suite and allows a feature-file change to escape affected-project selection.
...
- Editing the canonical corpus invalidates Nx caches and fails when structure, bindings, drivers, adapters, or behavior
  no longer agree.
