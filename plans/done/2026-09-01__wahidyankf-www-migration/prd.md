...
project's so that a behaviour change revalidates the same way regardless of which project it touches.
...
- Flattening the ose corpus into `specs/apps/wahidyankf-www/behaviours/` and authoring the as-built C4 model. The
...
These are the behaviours this plan adds. The existing corpus is preserved rather than re-specified;
...
- **[AC-3]** `npx nx run wahidyankf-www:test:coverage:integration` enforces 99% and passes against the two scenarios
  above.
- **[AC-4]** Every one of the twelve feature files in `specs/apps/wahidyankf-www/behaviours/` is loaded by exactly one
  binding — eleven from `apps/wahidyankf-www/tests/bdd/` and `cv-export.feature` from
  `apps/wahidyankf-www/tests/integration/` — proved by count, and `npx nx run wahidyankf-www:test:coverage:behaviour`
...
| `[AC-4]`  | Per-feature loader count, then `test:coverage:behaviour --skip-nx-cache`                                                 | All twelve features report exactly one loader; exit 0 with 12 files and 258 tests                                                                                      |
