...
Phase 2 adds `specs/apps/wahidyankf-www/behavior/` while the ose corpus continues to exist in its own repository. Phase
3 adds the application, the inlined modules, and `apps/wahidyankf-www/docs/`, all alongside the untouched `cv/`
...
disposition proof is behavioral rather than byte-comparative: the CV route must render every role that `cv-ats.md`
