#!/usr/bin/env node
/**
 * Single entrypoint that gives the Next.js app in this repo a stable runtime port contract: an
 * explicit `--port` flag wins, then the app's own prefixed environment variable, then its
 * compiled-in default.
 *
 * Why a wrapper exists at all: Next's CLI resolves its port from `--port` or a bare `PORT`, and an
 * explicit `--port` on the command line always outranks `PORT`. A `project.json` that passes
 * `--port <literal>` therefore means no environment variable can ever move the listener. The
 * standalone form is worse: Next's generated `server.js` reads `process.env.PORT` and parses no
 * flags at all. One wrapper that resolves the port first and then hands the answer to whichever
 * server shape follows is what makes every entry point agree.
 *
 * Usage:
 *   node scripts/next-with-port.mjs dev   --env WAHIDYANKF_WWW_PORT --default 3201 [--port 4000]
 *   node scripts/next-with-port.mjs start --env WAHIDYANKF_WWW_PORT --default 3201 [--port 4000]
 *   node scripts/next-with-port.mjs --env WAHIDYANKF_WWW_PORT --default 3201 --server apps/wahidyankf-www/server.js
 *
 * The `--server` form sets `process.env.PORT` and then imports the generated server, which is the
 * only knob that server exposes. No app here sets `output: "standalone"`, so nothing generates that
 * `server.js` today: the form is supported and unused rather than exercised.
 *
 ...
 * life. Signals are forwarded and re-raised so a supervisor still sees the conventional 128+N
 * status rather than a synthesised exit code.
 *
 * RESOLVER: the resolver is imported by a path relative to THIS file, so anything relocating this
 * wrapper must keep its layout against `apps/wahidyankf-www/src/features/env/core/port-resolver.ts`.
 * That module is deliberately dependency-free (no `dotenv`, no `node:fs`) so it needs no
 * `node_modules` — see its own header. Node strips the TypeScript types natively, so there is no
 * build step, which is what lets this `.mjs` file import a `.ts` module directly.
 */
...
import { resolvePort } from "../apps/wahidyankf-www/src/features/env/core/port-resolver.ts";
...
const mode =
  argv[0] !== undefined && !argv[0].startsWith("--") ? argv[0] : undefined;
...
if (serverPath !== undefined) {
  ...
  if (mode === undefined) {
    fail(
      "missing mode: expected `dev` or `start` (or --server <path> for a standalone image)",
    );
  }
  ...
  const nextBin = existsSync(localNext)
    ? localNext
    : existsSync(workspaceNext)
      ? workspaceNext
      : "next";
