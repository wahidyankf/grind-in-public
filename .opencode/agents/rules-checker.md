...
Your corpus is everything that can carry a rule: `repo-governance/`, `AGENTS.md`, `CLAUDE.md`, the `.agents/`, `.claude/`, `.codex/`, and `.opencode/` directories including every subagent and command definition, and every `SKILL.md`. Read `docs/`, `scripts/`, and the root `README.md` narrowly: only for a requirement that belongs in `repo-governance/` or a reference to something renamed, never for their style or for duplication among them. Read `plans/` and `specs/` for one property only — whether every directory at every depth carries a README that registers its immediate entries, counting every file type in `specs/` — and never for their content, which belongs to `plan-checker` and to the specs policy. Do not review source code.
